"""
League of Legends Win-Probability Data Collector
==================================================

Stratifies ranked solo/duo matches by skill bracket and extracts a game-state
snapshot every 60 seconds — Riot's native timeline frame interval — for
training a win-probability model. No interpolation: each row comes straight
from a real timeline frame. Objective counts are kept at team level; player-level
features include direct player state, combat, farm, position, KDA, vision, and full rune setup. Tower destruction state is tracked at the team/map level.

Rank tiers (each collected separately):
    IRON, BRONZE, SILVER, GOLD, PLATINUM, EMERALD, DIAMOND
    MASTER, GRANDMASTER, CHALLENGER

Pipeline:
    1. Pull league entries (summoners) per division/tier for each bracket.
    2. Resolve puuids, build a shuffled pool per bracket.
    3. Pull a few recent ranked-solo match ids per summoner until each
       bracket hits its target unique-match count.
    4. Fetch match + timeline for each match id (skips already-processed ids).
    5. Extract a feature row from every native 60s timeline frame and
       write chunked per-tier Parquet files.

Resumable: puuid pools and processed-match-id sets are cached to disk, so
re-running the script picks up where it left off instead of re-querying.

IMPORTANT — rate limits:
    Riot's *personal* dev API key (expires every 24h) is limited to roughly
    20 requests/1s and 100 requests/2min. Collecting ~600 matches x 10 rank
    tiers involves several thousand calls, which will take a while (or
    require applying for a production key with higher limits).
    Adjust RATE_LIMITS below to match whatever key you're using.
"""
...
class RateLimiter:
    def __init__(self, limits):
        self.limits = limits  # list of (max_calls, window_seconds)
...
def get_summoner_puuid(encrypted_summoner_id):
    """Fallback lookup if a league entry doesn't already carry a puuid."""
...
def collect_puuid_pool(bracket_name, target_pool_size, cache_path):
    ...
    if bracket_name in APEX_TIERS:
        ...
        per_division_cap = -(-target_pool_size // len(divisions))  # ceil division
...
# --------------------------------------------------------------------------
# Feature extraction: one row per native 60s timeline frame
# --------------------------------------------------------------------------
...
# Participant-frame scalar fields available from timeline participantFrames.
PLAYER_FRAME_FIELDS = (
    "currentGold",
    "totalGold",
    "level",
...
# Riot's participantFrame[damageStats] fields.
DAMAGE_STAT_FIELDS = (
    "magicDamageDone",
...
# Riot's participantFrame[championStats] fields.
...
# Event-derived cumulative per-player features.
# Objective ownership is intentionally kept at team level only.
# Riot timeline events often attribute objective kills to the last-hitting player,
# but for win-probability modeling those are strategic team outcomes, not stable
# player-level skill/state features.
PLAYER_EVENT_FIELDS = (
    "kills",
    "deaths",
    "assists",
...
# Diffs are added for the role matchup between Team 100 and Team 200.
ROLE_DIFF_FIELDS = (
    "currentGold",
    "totalGold",
    "level",
...
def _role_map(participants, team_ids):
    """role code ('top'/'jg'/'mid'/'bot'/'sup') -> participantId for one team."""
    result = {}
...
def _rune_setup(p):
    """Return full selected rune setup for one participant.

    Riot match-v5 stores the keystone and subrunes under perks.styles and
    the three stat shards under perks.statPerks. Missing values are kept as
    empty strings so the CSV schema stays stable.
    """
...
def _match_context(match):
    """Static, per-match info that doesn't change frame-to-frame."""
    info = match["info"]
...
def _build_tower_destroy_log(frames):
    """Return tower destruction events as (timestamp_ms, destroyed_team, lane, tier).

    For BUILDING_KILL tower events, Riot's event teamId is the team whose
    building was destroyed. This log therefore represents map state: which
    team's tower has been lost, not which team received credit.
    """
    tower_events = []
    for frame in frames:
...
def _tower_state_features(tower_destroy_log, up_to_ts):
    """Create 41 tower-state features for a timestamp.

    Convention: top_outer_100_destroyed = 1 means Team 100's top outer tower
    has been destroyed by this timestamp. Diff features use enemy-lost minus
    own-lost, so positive values favor Team 100.
    """
...
def _build_event_log(frames, team100_ids, team200_ids):
    """Build team-level and player-level event logs from timeline events."""
    team_event_log = []
    player_event_log = []
...
def _player_snapshot_values(pf, event_counts):
    """Return every per-frame and event-derived value for one participant."""
    if not pf:
        values = {}
...
def _add_role_diffs(row, role, vals100, vals200):
    ...
    # Backward-compatible aliases from the earlier CSV version.
...
BASE_CSV_FIELDS = [
    "match_id", "timestamp_sec", "game_duration_sec", "team_100_win",

    "patch",
    "team_100_champs", "team_200_champs",
    "team_100_summs", "team_200_summs",
    *[f"team_100_{rune_field}s" for rune_field in RUNE_PERK_FIELDS],
    *[f"team_200_{rune_field}s" for rune_field in RUNE_PERK_FIELDS],
    "team_100_bans", "team_200_bans",
    "first_blood_team", "first_blood_time_sec",
    "first_tower_team", "first_tower_time_sec",

    "gold_100", "gold_200", "gold_diff",
    "xp_100", "xp_200", "xp_diff",
    "cs_100", "cs_200", "cs_diff",

    "dmg_to_champs_100", "dmg_to_champs_200", "dmg_to_champs_diff",
    "hp_pct_100", "hp_pct_200", "hp_pct_diff",
    "armor_100", "armor_200",
    "mr_100", "mr_200",
    "ad_100", "ad_200",
    "ap_100", "ap_200",

    "kills_100", "kills_200", "kill_diff",
    "towers_100", "towers_200", "tower_diff",
    "plates_100", "plates_200", "plate_diff",
    "dragons_100", "dragons_200", "dragon_diff",
    "heralds_100", "heralds_200", "herald_diff",
    "barons_100", "barons_200", "baron_diff",
    "elders_100", "elders_200",

    "wards_placed_100", "wards_placed_200",
    "wards_killed_100", "wards_killed_200",
    "control_wards_100", "control_wards_200",

    *TOWER_STATE_FIELDS,
]


def _role_static_fields(role, team_id):
    prefix = f"{role}_{team_id}"
    return [
        f"{prefix}_participant_id",
        f"{prefix}_champ",
        f"{prefix}_champion_id",
        f"{prefix}_summoner1_id",
        f"{prefix}_summoner2_id",
        *[f"{prefix}_{rune_field}" for rune_field in RUNE_PERK_FIELDS],
    ]


def _role_player_fields(role, team_id):
    prefix = f"{role}_{team_id}"
    fields = []
    for field in PLAYER_FRAME_FIELDS:
        fields.append(f"{prefix}_{_snake(field)}")
    fields.append(f"{prefix}_cs")
    for field in DAMAGE_STAT_FIELDS:
        fields.append(f"{prefix}_{_snake(field)}")
    for field in CHAMPION_STAT_FIELDS:
        fields.append(f"{prefix}_{_snake(field)}")
    fields.extend([f"{prefix}_hp_pct", f"{prefix}_power_pct", f"{prefix}_x", f"{prefix}_y"])
    for field in PLAYER_EVENT_FIELDS:
        fields.append(f"{prefix}_{field}")
    return fields


def _role_diff_fields(role):
    fields = []
    for field in ROLE_DIFF_FIELDS:
        fields.append(f"{role}_{_snake(field)}_diff")
    fields.extend([f"{role}_gold_diff", f"{role}_dmg_diff"])
    return fields
...
def build_csv_fields():
    ...
    for role in ROLES:
        for team_id in TEAMS:
            fields.extend(_role_static_fields(role, team_id))
        for team_id in TEAMS:
            fields.extend(_role_player_fields(role, team_id))
        fields.extend(_role_diff_fields(role))

    # Preserve order while dropping accidental duplicates caused by aliases.
    seen = set()
    unique_fields = []
    for field in fields:
...
# Clean modeling schema supplied by final_clean_schema_no_tower_diffs_no_agg_stats.
# This intentionally removes redundant team-level concatenated draft/loadout strings,
# aggregate team stat totals, tower diff summaries, and other columns not listed in the schema.
...
# --------------------------------------------------------------------------
# Reduced nonredundant modeling schema
# --------------------------------------------------------------------------
...
# These fields are intentionally dropped from Parquet output because they are
# reconstructable from retained columns or are duplicate string/debug forms.
...
def process_bracket(bracket_name, global_processed):
    ...
    # oversample the puuid pool since some players won't have ranked games
    ...
    def flush_buffer():
        nonlocal part_index, buffered_rows, buffered_match_ids, processed, global_processed
